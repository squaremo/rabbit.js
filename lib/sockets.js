...
function Context(url) {
  ...
  c.on('close', function() { that.emit('close'); });
};
...
(function() {
  ...
  Context.prototype.close = function(callback) {
    ...
    var allclosed = latch(this._socks.length + 1,
                          c.end.bind(c));
...
function Socket(connection) {
  ...
  this._channels = []; // all channels we've created in the course of
...
(function() {
  ...
  proto.end = function(chunk, encoding, callback) {
    ...
    function finalise() {
      ...
      // below.
      var ended = latch(that._channels.length + 1, function() {
        that._invalidate();
        that.emit('end');
        that.emit('close');
      });
      ...
      var channel;
      while (channel = that._channels.shift()) {
        channel.once('close', ended);
        channel.close();
      }
      ended();
    }
  ...
  proto._advertise = function(exchange, routingKey, callback) {
    ...
    if (!exchange) {
      addToAdvertisements(
        this._register(this._connection.exchange()));
      this._register(
    ...
    if (this.writable) {
      ...
      if (ad) {
        ...
        if(this._opts.expiration)
          options.expiration = this._opts.expiration.toString();
  ...
  proto._consumeShared = function(queueName, callback) {
    var that = this;
    this._register(that._connection.queue(
  ...
  proto._consumePrivate = function(bindExchange, callback) {
    ...
    // NB bind is done async, since it's on the same channel as the
    // queue so it's implicitly serialised
...
RepSocket.prototype.connect = function(queue, callback) {
  ...
  this._await(function(done) {
    ...
    // cache that.
    that._exchange = that._register(that._connection.exchange(''));

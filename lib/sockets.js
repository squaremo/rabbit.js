...
(function() {
  ...
  Context.prototype.close = function(callback) {
    this._socks.forEach(function(s) { s.end(); });
...
(function() {
  ...
  proto.end = function() {
    ...
    this.writable = false;
    this._advertisements.forEach(function(advertisement){
      advertisement.exchange.close();
    })
  ...
  // Call when the underlying channel has blown up. We cannot expect
  // to either write or receive more data from the channel.
    ...
    if (!exchange) {
      this._regForErrors(
        this._connection.queue(
          ...
          function (q) {
            // NB not added to error handling

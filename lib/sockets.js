...
PubSocket.prototype.connect = function(destination, durable, callback) {
  var self = this, ch = this.ch;
  if (typeof durable === 'function') {
    callback = durable;
    durable = this.options.persistent || false;
  }
  else if (typeof durable === 'undefined') {
      durable = this.options.persistent || false;
  }
  ch.assertExchange(destination,
                    this.options.routing || 'fanout',
                    {durable: durable, autoDelete: false})
    .then(function(ok) {
...
SubSocket.prototype.connect = function(source, topic, durable, callback) {

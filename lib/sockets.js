...
RepSocket.prototype.write = function(chunk, encoding, corrId) {
  console.log('before', this.requests.length);
  var ch = this.ch,
      current = corrId ?
                  _.chain(this.requests)
                    .remove(_.findIndex(this.requests, ['properties.correlationId', corrId]), 1)
                    .head()
                    .value() :
                  this.requests.shift();
  console.log('after', this.requests.length);

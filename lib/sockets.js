...
function latch(num, callback) {
  return function() {
    if (--num === 0) callback &&callback();
  };
}

function Context(url) {
  EventEmitter.call(this);
  var socks = this._socks = [];
  var that = this;
  var c = this._connection = amqp.createConnection({url: url});
  c.on('ready', function() { that.emit('ready') });
  c.on('error', function(e) {
    socks.forEach(function(s) { s._closeWithError(e); });
    that.emit('error', e);
    that.emit('close');
  });
};
...
(function() {
  Context.prototype.socket = function(type) {
    ...
    if (Ctr) {
      var s = new Ctr(this._connection);
      var socks = this._socks;
      socks.push(s);
      s.once('close', function() {
        var i = socks.indexOf(s);
        socks.splice(i, 1);
      });
      return s;
    }
  ...
  Context.prototype.close = function(callback) {
    ...
    var allclosed = latch(this._socks.length,
                          c.end.bind(c));
    this._socks.forEach(function(s) {
      s.once('close', allclosed);
      s.end();
    });
  }
})();
...
function Socket(connection) {
  ...
  // this acts something like a readers-writer lock; we can't close
  // the socket until all in-progress operations are done.
  this._awaiting = 0, this._blocked = [];
...
(function() {
  ...
  // `_acceptOpt` for easy values, or this method for hard (e.g.,
  // multivalue) options.
  ...
  proto.write = function(data /*, encoding */) {
    ...
    if (!this.writable) throw new Error("Socket not writable");
  ...
  proto.end = function(chunk, encoding, callback) {
    // let this fail if we're closed
    if (chunk !== undefined) {
      proto.write(chunk, encoding);
      callback && callback();
    }
    ...
    var that = this;
    function finalise() {
      // All the ads closed, plus either close subs or explicit done
      // below.
      var ended = latch(that._advertisements.length + 1,
                        that.emit.bind(that, 'close'));
      
      if (that._advertisements.length > 0) {
        that._advertisements.forEach(function(advertisement) {
          var ex = advertisement.exchange;
          ex.once('close', ended);
          ex.close();
        });
      }
      that._advertisements = null;
      if (that.readable)
        that._cancel(ended);
      else
        ended();
  ...
  proto._await = function(work, cb) {
    var that = this;
    function done() {
      that._awaiting--;
      var fn; while (fn = that._blocked.shift()) fn();
      if (cb) cb();
    }
    that._awaiting++;
    work(done);
  ...
  // We can get an error back on any channel we use. NB this needs to
  // be done in sequence with creating the channel, not in a callback,
  // since many errors will arise from faulty declarations i.e.,
  // before the 'open' has completed.
  proto._regForErrors = function(chan) {
    var bang = this._closeWithError.bind(this);
    chan.on('error', bang);
  };

  // Call when the underlying connection has blown up. We cannot
  // expect to either write or receive more data.
  proto._closeWithError = function(err) {
    this._invalidate();
    this.emit('error', err);
    this.emit('close');
  };

  proto._invalidate = function() {
    this._closed = true;
    this.readable = this.writable = false;
    this._subscriptions = this._advertisements = null;
    this.connect = this.write = function() {
  ...
  proto._advertise = function(exchange, routingKey, callback) {
    ...
    if (!exchange) {
      // NB default exchange not added to error handling
  ...
  proto._sendOne = function(buf) {
    if (this.writable) {
      ...
      if (ad) {
        ...
        if(this._opts.expiration)
          options.expiration = this._opts.expiration;
  ...
  proto._consumeShared = function(queueName, callback) {
    var that = this;
    this._regForErrors(that._connection.queue(
  ...
  proto._cancel = function(callback) {
    ...
    for (var name in this._subscriptions) {
      subs.push(this._subscriptions[name]);
    }

    if (subs.length > 0) {
      var unsubbed = latch(subs.length, finish);
      subs.forEach(function(sub) {
        sub.queue.unsubscribe(sub.consumerTag).addCallback(unsubbed);
      });
    }
    else finish();
  };
...
PubSocket.prototype._acceptOpt =
  {'topic': true, expiration: true};

PubSocket.prototype.connect = function(exchange, callback) {
  var that = this;
  this._await(function(done) {
    that._advertise(
      exchange, '', function() {
        that.writable = true;
        done();
...
PubSocket.prototype._send = function(buf) {
  ...
  this._advertisements.forEach(function(ad) {
    ad.exchange.publish(rk, buf, options);
  });
  return true;
};
...
SubSocket.prototype.connect = function(exchange, callback) {
  var that = this;
  this._await(function(done) {
    that._consumePrivate(exchange, function() {
      that.readable = true;
      done();
    });
  }, callback);
...
PushSocket.prototype._acceptOpt =
  {expiration: true};
...
PullSocket.prototype.connect = function(addr, callback) {
  var that = this;
  this._await(function(done) {
    that._consumeShared(addr, function() {
      that.readable = true;
      done();
    });
  }, callback);
...
ReqSocket.prototype.connect = function(queue, callback) {
  var that = this;
  this._await(function(done) {
    that._consumePrivate(false, function() {
      that.readable = true;
      that._advertise('', queue, function() {
        that.writable = true;
        done();
...
RepSocket.prototype.connect = function(queue, callback) {
  ...
  this._await(function(done) {
    ...
    // cache that.
    that._exchange = that._connection.exchange('');
    that._consumeShared(queue, function() {
      that.writable = true;
      that.readable = true;
      done();
    });
